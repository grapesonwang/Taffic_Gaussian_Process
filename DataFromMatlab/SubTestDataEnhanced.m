function [] = SubTestDataEnhanced(output_folder, test_data,seq_ID_mat, common_index,boundry,TIME_SLOT,SEN_NUM)
%UNTITLED15 此处显示有关此函数的摘要
%   此处显示详细说明

%%
savepath = ['./ssdf_data/test_data/' output_folder] ;
if ~exist(savepath)
   mkdir(savepath);
else
    rmdir(savepath,'s');
...
for i = 1:1:sub_test_num
    ...
    xlswrite(strcat(strcat(savepath,'/',temp_name),'.xlsx'), temp_data, 'Sheet1');
end
